...
**Result scrolling** — `scroll_result_left` and `scroll_result_right` adjust `result_scroll_offset` within the range $0..\max(0, \text{last\_result\_len} - 13)$. The display shows 13 visible characters plus a scroll indicator arrow at column 14 when the result exceeds 15 characters. Overflow results formatted via `format_overflow` are always ≤14 characters (exponent capped at ±99), so they never need horizontal scrolling.
...
`handle_expression_submission` copies `input_buffer` into `expr_scratch`, then calls `engine::evaluate_expression`. The engine returns one of three `EvalResult` variants:

- **`Value(complex)`** — the result is within Q31.32 range. It is stored via `record_answer` and formatted through `engine::format_result` for display.
- **`Overflow { mantissa, exponent, negative }`** — the result exceeds Q31.32 range. `engine::format_overflow` renders it as scientific notation (e.g. `1.34406E+43`). If the exponent exceeds ±99, `format_overflow` returns `None` and the runtime displays `! overflow`.
- **`DomainError`** — an invalid input (e.g. `sqrt(-1)`). The runtime prints `! error`.

The input buffer is cleared and the OLED is refreshed with the result after every submission.
...
A scroll indicator arrow (left or right) is shown at column 14 when the result overflows. Overflow results are always short enough to fit without scrolling. Aggregate expressions (`sum(...)`, `int(...)`) use a special two-line layout with a tall glyph (integral or sigma symbol) spanning both pages, upper/lower bounds on separate pages, and the body to the right.
...
|----------|-------|---------|
| `0x01` | → | Right scroll arrow |
| `0x02` | ← | Left scroll arrow |
| `*` | × | Multiplication |
| `/` | ÷ | Division |
| `-` | − | Minus |
| `pi` digraph | π | Pi constant |
